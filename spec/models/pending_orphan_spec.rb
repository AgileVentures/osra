...
describe PendingOrphan, type: :model do
  ...
  specify '.create_orphan' do
    orphan = pending_orphan.instance_variable_get(:@orphan)
    expect(orphan.class.name).to eq 'Orphan'
  end

  specify '.to_orphan' do
  ...
  specify '.reject_fields_starting_with' do
    pending_orphan.reject_fields_starting_with(attributes, 'original_address')
    expect(pending_orphan.attributes[:current_address_city]).to be nil
  end
