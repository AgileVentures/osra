...
def and_i_should_see_filters_form_filled
  ...
  within "#filters" do
    ...
    [:name_value, :father_given_name_value, :family_name_value,
     ...
     :date_of_birth_from, :date_of_birth_until, :original_address_city,
    ...
    expect(page.find("select[name='filters[sponsorship_status]']"))
        .to have_selector("option[value='#{orphan_filter[:sponsorship_status]}'][selected]")
    expect(page.find("select[name='filters[status]']"))
        .to have_selector("option[value='#{orphan_filter[:status]}'][selected]")
    ...
    expect(page.find("select[name='filters[health_status]']"))
        .to have_selector("option[value='#{orphan_filter[:health_status]}'][selected]")
    ...
    expect(page).to have_select("filters[goes_to_school]", selected: (orphan_filter[:goes_to_school] ? "Yes" : "No"))
  end
end
...
def then_i_should_see_filters_form_clear
  within "#filters" do
    ...
    #select fields
    [:name_option, :father_given_name_option, :father_given_name_value,
     :gender, :province_code, :sponsorship_status, :status,
     :father_is_martyr, :mother_alive, :goes_to_school]. each do |field|
