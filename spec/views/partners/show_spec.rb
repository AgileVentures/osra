  ...
  describe 'instance action-items' do
    ...
    describe 'Orphan Lists button' do
      it 'is disabled when lists absent' do
        render
        expect(rendered).to have_selector("a[href='#{hq_partner_orphan_lists_path(partner.id)}'][disabled]",
          :text => "Orphan lists")
      end
      ...
      it 'is enabled when lists present' do
        ...
        render
        expect(rendered).to have_selector("a[href='#{hq_partner_orphan_lists_path(partner.id)}']",
          :text => "Orphan lists")
