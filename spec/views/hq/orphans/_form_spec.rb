...
RSpec.describe "hq/orphans/_form.html.erb", type: :view do
  ...
  before :each do
    assign(:orphan_statuses, orphan_statuses)
    assign(:orphan_sponsorship_statuses, orphan_sponsorship_statuses)
  ...
  specify 'form values' do
    ...
    assert_select "select#orphan_original_address" do
    ...
    assert_select "select#orphan_current_address" do
      assert_select "option", value: provinces.first.id,
                               html: CGI::escape_html(provinces.first.name)
    end
